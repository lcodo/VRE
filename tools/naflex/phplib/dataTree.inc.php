...
#
	...
	ob_start();?>
<tr><td valign=top class='toolboxtitle'><a href = '<?=$leaf?>' name='<?=$leaf?>'</a><?=indentat($tree, $leaf,1)?>
<?	if (count($data['leaf'])) {?>
		<a href=javascript:submitAs('project','t_<?=$leaf?>')><?=$icon[$data['estat']]?></a>
<?}	else {?>
		<?=$punt?>
	<?}?>
	<?=$typeIcon[$data['objectType']]?><a href="javascript:setVis('<?=$leaf?>')"><?=$title?></a> <?if ($data['Descript']) print '('.$data['Descript'].')'?> <?if ($data['config']) print '(MD Config Files)'?> <?if(!$data['config']) print '('.getSize($tree, $leaf).')'?> <?=$statIcon[$data['fileok']]?>
     <div id="<?=$leaf?>Div" class="toolbox">
    <?if ($data['fileok'] == 1) {
        ...
        if  ($toolboxType) {
			foreach ($toolboxType as $op) {
				if($op == "jmolTraj") {?>
			 		<a href= "javascript:alert('Visualize a trajectory could take some minutes. Please, be patient.');javascript:submitAs('project','<?=$op?>_<?=$leaf?>')"><?=$opsIcons[$op]?></a>
				<? } else {?>
					<a href=javascript:submitAs('project','<?=$op?>_<?=$leaf?>')><?=$opsIcons[$op]?></a>
				<?}
			}
		}
	} elseif ($data['fileok'] == 3) {?>
		<a href=javascript:submitAs('project','readLog_<?=$leaf?>')><?=$opsIcons['readLog']?></a> <a href=javascript:submitAs('project','trash_<?=$leaf?>')><?=$opsIcons['trash']?></a>
	<?  
	} else {?>
    	<a href=javascript:submitAs('project','trash_<?=$leaf?>')><?=$opsIcons['trash']?></a>
    <?}?>
    </div>
</td></tr>
<?if ($ops['leaf']==$leaf)  { # cal obrir Ops
#	$newId = newFileOrder($_SESSION[projectData][projDir]);
	$newId = newObjectOrder($tree);
    ?>
	<tr><td> <div id="opsDialog">
		<input name="leaf" type="hidden" value="<?=$leaf?>" />
		<input name="newId" type="hidden" value="<?=$newId?>" />
       	<p>Select the desired operation.</p>
       	<p>Title: <input name="titol" size="25" value="_<?=$newId?>" />
	...
	<? if(!empty($ops['ops'])){?>
	  <select name="idOperacio" onChange='setHelpForm();setInnerForm()'>  <option value="XX">List of Operations:</option> 
       	<?  foreach ($ops['ops'] as $o) {?> 
        	<option value="<?=$o['idOperacio']."-".$o['subtype']?>"><?=$o['nom']?></option> 
	  <? } ?>
		</select>
	...
	<tr><td> 
		...
		<div id="opsDialog">
       	<input type="button" value="ok" onClick='checkParams(error,"project","doMoby_<?=$leaf?>")'/>
...
<?}	else {?>
		<?=$punt?>
	<?}?>
	<?=$typeIcon[$data['objectType']]?><a href="javascript:setVis('<?=$leaf?>')"><?=$title?></a> <?if ($data['Descript']) print '('.$data['Descript'].')'?><? print '('.getSize($tree, $leaf).')'?> <?=$statIcon[$data['fileok']]?>
     <div id="<?=$leaf?>Div" class="toolbox">
    <?if ($data['fileok'] == 1) {
