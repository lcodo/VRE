...
?>
<script>
newWin = window.open('<?=$f[baseURL]."/".$data[$leaf][fname].".$fileExt"?>','<?=$leaf?>')
</script>
<?}
...
function sendPlot (&$f, $leaf, $op) {
	...
	print headerNA("NAFlex: Nucleic Acids Flexibility",0);
	#print headerMMB("MDWeb. Molecular Dynamics on Web. ".$data[$leaf]['fname'], 0);?>
    <h4><? echo $t; echo "\t( $units )" ?><?if ($data[$leaf]['Descript']) print "(".$data[$leaf]['Descript'].")"?></h4>
    <p align="center"><img border="1" src="<?=$data[$leaf]['fname'].".png"?>"></p>
    <?print footerNA();
	$txt =ob_get_contents();
	ob_end_clean();
...
#                $result = execScript("checkSnapshot.pl", $f['projDir']."/".$f['fileTree'][$leaf]['fname']." 16");
		...
		logger("Number of Snapshots: $n");
			...
			logger("http://mmb.pcb.ub.es/FlexServ/analysis2.php?analysisType=MDWeb&user=$login&project=$projID&structure=$structname&trajectory=$trajname");
                ?>
                <script>
                    newWin = window.open('<?="http://mmb.pcb.ub.es/FlexServ/analysis2.php?analysisType=MDWeb&user=$login&project=$projID&structure=$structname&trajectory=$trajname" ?>','<?=$leaf?>flexanal')
                </script>
                <?
...
function getOpsSecondaryHTML($o) { ## Possible formulari automatic???
    ...
    <?if ($o['subtype']) {?>
    <input type="hidden" name="prm[md_type]" value="<?=$o['subtype']?>" />
    <?}
//print_r($o);
    foreach (array_values($o['secondary']) as $prm) {
        ...
        if ($prm['type'] == "secondary") {
          if ($prm['articleName'] == "persistent") {
            ...
            ?>
            	<input type="hidden" value="true" name="prm[<?=$prm['articleName']?>]" />
            <?
              } 
            ...
            switch ($prm['objectName']) { 
                case 'Integer':
                    case 'Float':?>
                        <?=$prm['articleName'] ?> <input size="10" name="prm[<?=$prm['articleName']?>]" value="<?=$default?>" />
        <?               break;
                    case 'String':
                        ...
                        if ($enum && ($enum != 'NULL')) {
                            ...
                            echo $prm['articleName']." ";
                             ...
                             ?><select name="prm[<?=$prm['articleName']?>]">
                             <?foreach ($enumarr as $k=>$v) {
                                 if ($v == $default) {
                        ...
                        ?>
                        <?=$prm['articleName'] ?> <input size="30" name="prm[<?=$prm['articleName']?>]" value="<?=$default?>" />
                        <?
                        }
                        break;
                    case 'Boolean':
                    ...
                    ?>
                    <?=$prm['articleName'] ?> <input type="checkbox" checked="checked" name="prm[<?=$prm['articleName']?>]" />
                    <?}
                    else {
                        ?>
                        <?=$prm['articleName'] ?> <input type="checkbox" name="prm[<?=$prm['articleName']?>]" />
                    <?}
             }
          }
        }
